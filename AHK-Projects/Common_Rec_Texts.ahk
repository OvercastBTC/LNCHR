...
ControlGet, cCtl, hWnd,,,ahk_pid %hwndChild%
...
;NumPut(0x80, PBYTE2, 0x42, "UChar") ;VK_B := 0x42
...
:*:recresp::
Clipboard:= ""
txt:= "Management stated they understand the recommendation. When the recommendation is received, the vendor will be engaged to ascertain the scope, cost, and scheduling. No timeline for completion was offered."
Clipboard:= % txt
ClipWait 1
send ^v
Sleep 100
...
:*:lvtit::
Clipboard:=""
txt:= "Improve asset management of the low voltage switchgear."
Clipboard:= % txt
ClipWait 1
Send ^v
Sleep 100
...
Clipboard:=""
Sleep 100
txt:= "Improve asset management of the medium voltage switchgear."
Clipboard:= % txt
ClipWait 1
Send ^v
sleep 100
...
Clipboard:=""
Sleep 100
txt:= "Perform annual online partial discharge (PD) surveys for switchgear and circuit breakers rated 4 kV and above per "
Clipboard:= % txt
ClipWait 1
Send ^v
Sleep 100
...
Send, .`n`n`n
Clipboard:=""
txt:= "Note: This recommendation is being made due to updated guidance, and associated industry loss history."
Clipboard:= % txt
ClipWait 1
Send ^v
Sleep 100
...
:*:pduth::
Clipboard:= ""
txt:= "Partial discharge (PD) and corona activity occur in switchgear rated 4 kV and above and is a good indicator of insulation issues due to incorrect installation, manufacturer defects, and signs of aging. The switchgear protects the electrical distribution and downstream production equipment."
Clipboard:= % txt
ClipWait 1
Send ^v
Sleep 100
...
Clipboard:= ""
Sleep 100
txt:= "Improve inspection, testing, and maintenance (ITM) of medium voltage (MV) switchgear batteries."
Clipboard:= % txt
ClipWait 1
Send ^v
Sleep 100
...
Clipboard:= ""
Sleep 100
txt:= "Improve asset management of medium voltage (MV) switchgear batteries."
Clipboard:= % txt
ClipWait 1
Send ^v
Sleep 100
...
Clipboard:=""
Sleep 100
txt:= "Improve the management of medium voltage (MV) switchgear batteries per "
Clipboard:= % txt
ClipWait 1
Send ^v
Sleep 100
...
:*:ecptm::
	MsgBox, 35, ,(insert asset(s) here)???
	IfMsgBox,Yes
		MsgBox, 35,,Improve (Yes) or Create (No)?
	IfMsgBox,Yes
		ImpCrt:= "Improve the "
	else IfMsgBox, No
		ImpCrt:= "Create an"
	...
	enterasset:
	IfMsgBox, Yes
		MsgBox,35, Input Asset Now?
	IfMsgBox,Yes
		InputBox, asset,,Enter Asset
	Gosub,SingleOrMulti
	return
	IfMsgBox No
		asset:= "(!!!***insert critical asset here***!!!)"
	Gosub,ECPClip
	return
	return
	
	SingleOrMulti:
	MsgBox,35,,Single (Yes) or Multi (No)?
	IfMsgBox, Yes
		Gosub,Single
	return
	IfMsgBox No
...
:*:ecpts::
	Title:= "Improve the equipment contingency plan (ECP) for the (insert asset here)."
	Clipboard:= txt
	ClipWait 1
	Send ^v
	Sleep 100
	...
	Single:
	Clipboard:= ""
	txt:= %ImCrt% "equipment contingency plan (ECP) for the " %asset% " per "
	Clipboard:= % txt
	ClipWait 1
	Send ^v
	Sleep 100
	...
	Multi:
	Clipboard:= ""
	txt:= %ImCrt% "equipment contingency plans (ECPs) for the " %asset% " per "
	Clipboard:= % txt
	ClipWait 1
	Send ^v
	Sleep 100
	...
	;^+#l::
	;HCtrl:="testtesttest"
	;RichEdit_SetCharFormat(HCtrl,Face	 = 	"",Style	 = 	italic,TextColor	 = 	"",BackColor	 = 	"",Mode	 = 	"SELECTION"	)
	;MsgBox % return
	/*
