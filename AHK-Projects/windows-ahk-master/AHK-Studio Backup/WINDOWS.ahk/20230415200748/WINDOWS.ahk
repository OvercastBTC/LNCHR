...
; FUNCTIONS
#IfWinExist ahk_id AHK Script.ahk
	ahks:= ahk_id AHK Script.ahk
	winget, outputvar, ID, ahk_id AHK Script.ahk
	if (outputvar = ahks)
		return
	if (outputvar = false)
		^+!r::
...
runAHKScript(){
	; run Scriptlet_Library_v4
	Run, AHK Script.ahk, C:\AHK-Studio\AHK-Projects\
}
runWinspector(){
	; run WinSpectorU.exe
...
runtxtundrmouse(){
	...
	Run, GetTextUnderMouse.ahk, C:\AHK-Studio\AHK-Projects\UIA
}
...
updateTrayMenu() {
	...
	addTrayMenuOption("Modified with Nerd by Adam Bacon.`nCredit:Made with ❤️ by Bibek Aryal", "madeBy")
	addTrayMenuOption()
...
updateTrayMenu()
trayNotify(script_full_name . " started", "Open keyboard shortcuts with {Ctrl + Shift + Alt + \}`n`nModified with Nerd by Adam Bacon`nMade with ❤️ by Bibek Aryal.")
...
return
#+e::Run, C:\AHK-Studio\AHK-Projects\ahk script.ahk
...
FormatSeconds(tSaved, tStart){  ; Convert the specified number of seconds to hh:mm:ss format.
	
	StartTime:= tStart
	EndTime:= tSaved
	DeltaT:= StartTime - EndTime
	ElapsedHours := SubStr(0 DeltaT /= 3600,1,2)
	ElapsedMinutes := SubStr(0 DeltaT /= 360,4,2)
	ElapsedSeconds := SubStr(DeltaT /= 60, -1,2)
	ElapsedMilliseconds := ElapsedSeconds / 1000
	return % ElapsedHours " hours : " ElapsedMinutes " minutes : `r" ElapsedSeconds " seconds" ;ElapsedMilliseconds " milliseconds"
	;ElapsedHours := SubStr(0 ElapsedSeconds / 60,1)
	;ElapsedMinutes := SubStr(0 ElapsedSeconds / 60,1)
	;ElapsedSeconds := SubStr (DeltaT,1)
	;ElapsedMilliseconds := ElapsedSeconds / 1000
	;return % ElapsedHours " hours : " ElapsedMinutes " minutes : `r" ElapsedSeconds " seconds" ;ElapsedMilliseconds " milliseconds"
	;msgbox % DeltaT
	;ElapsedHours := SubStr(0 (DeltaT / 3600000), -1)
	;ElapsedMinutes := SubStr(0 ((DeltaT - ElapsedHours * 3600000) / 60000), -1)
	;ElapsedSeconds := SubStr(0 Floor((DeltaT - ElapsedHours * 3600000 - ElapsedMinutes * 60000) / 1000), -1)
	;return % ElapsedHours " days : " ElapsedMinutes " hours : `r" ElapsedSeconds " minutes : " ElapsedMilliseconds " seconds"
}
timeDiff(startTime, finishTime) {
	starthour := SubStr(startTime, 9, 2)
	startminute := SubStr(startTime, 11, 2)
	startsecond := SubStr(startTime, 13, 2)
	start := starthour ":" startminute ":" startsecond ;20000101 starthour startminute "00"
	MsgBox % start
	
	finishhour := SubStr(finishTime, 9, 2)
	finishminute := SubStr(finishTime, 11, 2)
	finishsecond := SubStr(finishTime, 13, 2)
	
	finish :=  finishhour ":" finishminute ":" finishsecond ;20000101 finishhour finishminute "00"
	...
	if (Th !> 1 and Tm !>1)
		Return LastSaved:= %Ts%
	If (Th !> 1 and Tm > 1)
		Return LastSaved:= %Tm% ":" %Ts%
	If (Th > 1 and Tm > 1)
		Return LastSaved:= %Th% ":" %Tm% ":" %Ts% 
	LastSaved:= Ts "secs" or LastSaved:= Tm "mins:" Ts "secs" or LastSaved:= Th "hrs:" Tm "mins:" Ts "secs"
	MsgBox % LastSaved
	return, LastSaved
}
CountDown(title = "", delay = ""){
	SetTimer, disable_enable , %delay%
...
^F8::
    random, rnd, 1, 100
    new SelfDeletingTimer(5000, "test", rnd)
return
*/
