...
;#Persistent ; Keeps script permanently running1#SingleInstance,Force
...
DockToWindow:
  ...
  ;get editor position
    WinGetTitle, ActiveWindowTitle, %A_ScriptName%
...
ChangeFont:
  Gui, 1:+OwnDialogs
  InputBox, OutVar, Choose Font, Specify an existing font name`nfor the scriptlet window (Default: Times New Roman),,270,130,,,,, %StrFontName%
  If ErrorLevel 
      Return
  StrFontName = %OutVar%
  GoSub, ReRunOrReload
...
ChangeFontSize:
  Gui, 1:+OwnDialogs
  InputBox, OutVar, Choose Font Size, Specify a font size for`nthe scriptlet window (Default: 1),,250,130,,,,, %IntFontSize%
  If ErrorLevel 
      Return
  IntFontSize = %OutVar%
  GoSub, ReRunOrReload
