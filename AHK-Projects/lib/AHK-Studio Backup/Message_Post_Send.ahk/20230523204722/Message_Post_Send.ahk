...
PostMessage, WM_GETOBJECT:=0x003d, 0xffffffff, 0xfffffffc, % ctl, A
;count:=DllCall("SendMessage","PTR",ctl,"UInt",B1,"PTR*",start,"PTR*",end) ;EM_SETSEL:=0xB1
;Clipboard:= DllCall("SendMessage","PTR",ctl,"UInt",0xB0,"PTR*",start,"PTR*",end) ;EM_GETSEL:=0xB0
;return
;Count:= SendMessage, CB_GETCOUNT:=0x146, 0,0,%hctl%, ahk_id %Ctl%
;MsgBox % "Count: "count ": ErrorLevel " ErrorLevel
return
#1::
#Persistent
;ToolTip
;SendMessage,0x0081,0,0,% Ctl,% hCtl
OnMessage(0x003d, "MyMessageMonitor")
...
#8::
;SetTimer, WatchCursor8, % (Toggle := !Toggle) ? 70 : "Off"
