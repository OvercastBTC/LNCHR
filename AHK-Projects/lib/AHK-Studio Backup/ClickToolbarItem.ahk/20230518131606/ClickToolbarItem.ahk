...
;vCtlClassNN:=dllCall("GetClassInfoEx","UInt",hwnd,"Str",class,"Ptr",WNDCLASSEX)
...
ToolTip `nClass?: %vCtlClassNN% Control: %hCtl% ahk_id %ctl%`nFocus: %fCtl%`nName: %nCtl%`rX: %x% Y: %y%, x+1, y+1
Return
...
#3::
ControlGet, hToolbar, hWnd,,%gToolbarWind%, A
ClickToolbarItem(hToolbar,3)
return

;/* ; rev2
#6::
;gToolbarWind:= "ThunderRT6FormDC1"
;ControlGet, hToolbar, hWnd,,%gToolbarWind%, A
...
ToolbarInfo(hWndToolbar, n){
	static TB_BUTTONCOUNT := 0x418, TB_GETBUTTON := 0x417, TB_GETITEMRECT := 0x41D, TB_COMMANDTOINDEX := 0x419
	...
	if (n >= 1 && n <= buttonCount) {
		...
		SendMessage, TB_COMMANDTOINDEX, % n-1, remoteMemory, , ahk_id %hWndToolbar%
