...
*/
#3::
hkLabel:= StrSplit(%A_ThisHotkey%,#,#)
WatchCursor:= "WatchCursor" %hkLabel%
SetTimer, % WatchCursor, % (Toggle:=!Toggle) ? "Off" : "On"
If Toggle
	ToolTip,,,,% ToolTipNum
return
%WatchCursor%:
MouseGetPos,CursorX,CursorY,vWin, hCtl
ControlGet, vhWnd, hWnd,, %hCtl%, A

win := WinEnum(%vhWnd%) ; calls EnumWindows
children := WinEnum(%vhWnd%) ; enumerate child windows of the active window
;MsgBox % "Win: " win " | " "Children: " children
wt:= DllCall("WindowFromPoint", "int64", CursorX | (CursorY << 32), "Ptr")
CursorHwnd := DllCall("WindowFromPoint", "int64", CursorX | (CursorY << 32), "Ptr")
MouseGetPos,x,y , id, gControl
WinGetTitle, title, A
WinGetClass, class, A
ControlGetFocus, ActiveWin, A
ControlGet, List,List,,,ahk_id %wt%
ControlGet, Checked,Checked,,,ahk_id %wt%
ControlGet, Enabled,Enabled,,,ahk_id %wt%
ControlGet, Visible,Visible,,,ahk_id %wt%
DllCall("SendMessage","PTR",hWnd,"UInt",0x1304, Ptr, CursorX, Ptr, CursorY) ; 0x1304 is TCM_GETITEMCOUNT.
TabCount := ErrorLevel

ControlGet, Tab,Tab,,% control,ahk_id %wt%
ControlGet, FindString,FindString,%control%,% hCtl,ahk_id %wt%
ControlGet, Choice,Choice,,,ahk_id %wt%
ControlGet, LineCount,LineCount,,,ahk_id %wt%
ControlGet, CurrentLine,CurrentLine,,% control,ahk_id %wt%
ControlGet, CurrentCol,CurrentCol,,,ahk_id %wt%
ControlGet, Line,Line,1,% Control,ahk_id %wt%
ControlGet, Selected,Selected,,,ahk_id %wt%
ControlGet, Style,Style,,,ahk_id %wt%
ControlGet, ExStyle,ExStyle,,,ahk_id %wt%
ControlGet, Hwnd,Hwnd,,,ahk_id %wt%
Control:= % hCtl

vToggleCopy:="Press Ctrl+Win+t to toggle this OFF|ON `n Press Alt-F12 to Copy the contents to the clipboard."
ToolTip % "TabCount: " TabCount "`nFindString: " FindString "`nChoice: " Choice "`nLineCount: " LineCount "`nCurrentLine: " CurrentLine "`nCurrentCol: " CurrentCol "`nLine: " Line "`nSelected: " Selected "`nStyle: " Style "`nExStyle: " ExStyle "`nHwnd: " Hwnd "`nCursorX: " CursorX "`nCursorY: " CursorY "`nList: " List "`nChecked: " Checked "`nEnabled: " Enabled "`nVisible: " Visible "`nTab: " Tab "`nHwnd: " Hwnd "`nx: " x "`ny: " y "`nTitle: " title "`n" "Unique ID: ahk_id " id "`n" "Class: ahk_class " class "`n" "Control: " gControl "`n" "Active Control: " ActiveWin "`n" "`n----------------------------------" "`nInstructions:`n" vToggleCopy "`n" "Win: " win " | " "Children: " children "`nvWin: " vWin " | vhWnd: " vhWnd "`nwt: " wt,5,0,%ToolTipNum%
...
^#T:: 
SetTimer, WatchCursor%hkLabel%, % (Toggle:=!Toggle) ? "Off" : "On"
If Toggle
	ToolTip,,,,% %hkLabel%
Return
 ...
 */
WinEnum(hwnd, lParam:=0) ;// lParam (internal, used by callback)
{
	static pWinEnum := "X"
...
ClickToolbarItem(hWndToolbar, n){
	...
	if (n >= 1 && n <= buttonCount) {
		...
		ToolTip, The specified index %n% is out of range. Please specify a valid index between 1 and %buttonCount%.,,,55
	}
	return
}
reload
