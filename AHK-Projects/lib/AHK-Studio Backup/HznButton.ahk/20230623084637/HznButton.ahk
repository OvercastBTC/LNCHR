...
ControlGetFocus, hCtl, A
WinGetClass, vhCtl, % "ahk_id " hCtl
ControlGet, vCtl, hWnd,,% hCtl, A
MsgBox % "1: " hCtl "`n2: " vCtl
ControlGet, yCtl, hWnd,,% hCtl, A

;MsgBox % "xCtl: " xCtl  "`nyCtl: " yCtl
;hWndChild := DllCall("RealChildWindowFromPoint","Ptr",Ctl,"UInt",4,"Ptr")
;xhWndChild := DllCall("RealChildWindowFromPoint","Ptr",xCtl,"UInt",4,"Ptr")
;yhWndChild := DllCall("RealChildWindowFromPoint","Ptr",yCtl,"UInt",4,"Ptr")
;VarSetCapacity(cClass, 261*2, 0)
;DllCall("GetClassName","Ptr",Ctl,"Str",cClass,"Int",261)
VarSetCapacity(xcClass, 261*2, 0)
DllCall("GetClassName","Ptr",xCtl,"Str",xcClass,"Int",261)
;VarSetCapacity(ycClass, 261*2, 0)
;DllCall("GetClassName","Ptr",yCtl,"Str",ycClass,"Int",261)

;WinGetClass, vCtl, % "ahk_id " hWndChild
;WinGetClass, xvCtl, % "ahk_id " xhWndChild
;WinGetClass, yvCtl, % "ahk_id " yhWndChild
;WinGetClass, vPCtl, % "ahk_id " hWnd
DllCall("SendMessage","Ptr",vCtl,"UInt",0xB1, "Ptr", 0, "Ptr", -1)
;DllCall("SendMessage","Ptr",Ctl,"UInt",0xB1)

;if (ErrorLevel != 1) {
;DllCall("SendMessage","Ptr",Ctl,"UInt",0xB1)
;MsgBox 0x40010, Error, % "SendMessge Error: " ErrorLevel "`n" vCtl "`n" cClass
;}
ToolTip % "xvCtl: " xvCtl " | vCtl: " vCtl "`nxcClass: " xcClass " | ycClass: " ycClass,,,
/*
If(vCtl = "ThunderRT6TextBox"){
	DllCall("SendMessage","Ptr",Ctl,"UInt",0xB1, "Ptr", 0, "Ptr", -1) ;EM_SETSEL:=0xB1
}
If(vCtl = "TX11"){
	DllCall("SendMessage","Ptr",Ctl,"UInt",0xB1)
} ;else {
...
*/
/*
vhCtl := vCtl = "ThunderRT6TextBox" ? DllCall("SendMessage","Ptr",Ctl,"UInt",0xB1)
	  : vCtl = "TX11"			 ? DllCall("SendMessage","Ptr",Ctl,"UInt",0xB1)
							 : DllCall("SendMessage","Ptr",Ctl,"UInt",0xB1, "Ptr", 0, "Ptr", -1)
sleep 1000
ToolTip
*/
