...
ControlGetFocus, fCtl, A
bID:= SubStr(fCtl, 0, 1)
ControlGet, hTbCtl, hWnd,,% "msvb_lib_toolbar" bID, A
;WinGetClass, TbCtlClass, % "ahk_id " hTbCtl
;MsgBox % hTbCtl " " TbCtlClass

;MsgBox % hTbCtl ? "Toolbar handle: " hTbCtl "`nButton count: " ErrorLevel : "Didn't find toolbar!"
EnumToolbarButtons(hTbCtl)
...
EnumToolbarButtons(ctrlhwnd, is_apply_scale:=false) {
	...
	*/
	...
	Loop,%buttons%
	{
		...
		idButton := NumGet(BtnStruct, 4, "UIntP")
		...
		SendMessage, TB_COMMANDTOINDEX := 1049,% idButton,0,, % "ahk_id " ctrlhwnd ; hope that 4KB is enough ; just a test
		btnvar1 := ErrorLevel
		;043f		1087		TB_GETBUTTONINFOW
		SendMessage, TB_GETBUTTONINFO := 1087,% btnvar1,remote_buffer,, % "ahk_id " ctrlhwnd ; hope that 4KB is enough ; just a test
		;btnvar2 := ErrorLevel
		;MsgBox, 1, , % btntip,1
		SendMessage, % TB_GETSTATE, % idButton, 0, , % "ahk_id " ctrlhwnd ; hope that 4KB is enough ; just a test
		btnstate := ErrorLevel
		;MsgBox, 1, , % btnstate,1
		SendMessage, % TB_GETBUTTONTEXTW, % btnvar1, remote_buffer, , % "ahk_id " ctrlhwnd ; hope that 4KB is enough
		...
		if(btntextchars>0){
			...
			BtnText := StrGet(&BtnTextBuf)
		} else {
			BtnText := ""
		}
		if(btnvar2>0){
			btnvar2bytes := A_IsUnicode ? btnvar2*2 : btnvar2
			VarSetCapacity(BtnVar2Buf, btnvar2bytes+2, 0) ; +2 is for trailing-NUL
			ReadRemoteBuffer(hpRemote, remote_buffer, BtnVar2Buf, btnvar2bytes)
			BtnVar2 := StrGet(&BtnVar2Buf)
		} else {
			BtnVar2 := ""
		}
		...
		FileAppend, % A_Index . ":" . idButton . "(" . btntextchars . ")" . BtnText .  " State: " btnstate . " C2I: " Btnvar1 . " Info: " Btnvar2 . " X1: " x1 . " X2: " x2 . " Y1: " y1 . " Y2: " y2 . " ErrorLevel: " ErrorLevel . "`n", _emeditor_toolbar_buttons.txt ; debug
		ToolTip,  % A_Index . ":" . idButton . "(" . btntextchars . ")" . " Txt: " BtnText . " State: " btnstate . " C2I: " Btnvar1 . " Info: " Btnvar2 . " X1: " x1 . " X2: " x2 . " Y1: " y1 . " Y2: " y2 . " ErrorLevel: " ErrorLevel . "`n" ;, _emeditor_toolbar_buttons.txt ; debug
		...
		arbtn := ( {"x":x1, "y":y1, "w":x2-x1, "h":y2-y1, "cmd":idButton, "text":BtnText} )
		...
		;arbtn.InsertAt( {"x":x1, "y":y1, "w":x2-x1, "h":y2-y1, "cmd":idButton, "text":BtnText} )
